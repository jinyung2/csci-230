/*  Java Class: HybridSort.java
    Author: Jin Choi
    Class: CSCI 230
    Date: April 18, 2018
    Description: Uses merge sort until the size is less than 21, in which case it performs insertion sort.
...
import java.util.Comparator;
 ...
 public class HybridSort<K,V> extends AbstractSort<K,V> {

        public HybridSort(int size){
            super(size);
            setSortingMethod("Hybrid");
        }
        ...
        private void merge(MapEntry<K,V>[] S1, MapEntry<K,V>[] S2, MapEntry<K,V>[] S, Comparator<K> comp) {
            int i = 0, j = 0;
            while (i + j < S.length) {
     ...
     private void insertionSort(MapEntry<K, V>[] entries, Comparator<K> comp) {

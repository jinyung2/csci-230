/*  Java Class: InsertionSort.java
    ...
    Class: CSCI 230
    Date: Mar 18, 2018
    Description: Merge sort implemented using the book code. Modified certain things (added a Integer comparator to use as a parameter) and a fill method to input random entries. Test driver is within class since provided code is given as static classes.

    I certify that the code below is my own work.

	Exception(s): Editted Book Code
...
class MergeSort {
  // NOT THE ORIGINAL CODE, EDITED TO RETURN THE ARRAY AS OUTPUT//
  ...
  //NOT ORIGINAL CODE, EDITED TO RETURN ARRAY//
  ...
  public static <K> K[] mergeSort(K[] S, Comparator<K> comp) {
    ...
    // merge results
    finalArray = merge(S1, S2, S, comp);               // merge sorted halves back into original
    return finalArray;
  }
  ...
  public static <K> void print(K[] array){
  ...
  public static void fillInt(Integer[] array){
  ...
  public static void main(String[] args) {
    Integer[] test = {10,1,9,2,8,3,7,4,6,5};
    System.out.print("Current contents of array: ");
    print(test);
    IntComparator comp = new IntComparator();
    mergeSort(test, comp);
    System.out.print("Contents after merge sort: ");
    print(test);

    //Testing run time using nanoTime for Extra Credit.//

    Integer[] oneThousand = new Integer[1000];
    Integer[] tenThousand = new Integer[10000];
    Integer[] hundredThousand = new Integer[100000];
    fillInt(oneThousand);
    fillInt(tenThousand);
    fillInt(hundredThousand); //filling up the test cases with random Integer entries.
    System.out.println("\nTesting Extra Credit cases for 1000, 10000 and 100000 entries using Merge Sort: ");
    long startTime = System.nanoTime();
    mergeSort(oneThousand, comp);
    long endTime = System.nanoTime();
    System.out.println("Run time (1000 entries): " + (endTime - startTime)/1000000.0 + " ms");
    startTime = System.nanoTime();
    mergeSort(tenThousand, comp);
    endTime = System.nanoTime();
    System.out.println("Run time (10000 entries): " + (endTime - startTime)/1000000.0 + " ms");
    startTime = System.nanoTime();
    mergeSort(hundredThousand, comp);
    endTime = System.nanoTime();
    System.out.println("Run time (100000 entries): " + (endTime - startTime)/1000000.0 + " ms");
    mergeSortBottomUp(test, comp);
  }
}

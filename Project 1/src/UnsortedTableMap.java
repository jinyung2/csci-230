/*  Java Class:  UnsortedTableMap.java
    Author: Jin Choi
    Class: CSCI 230
    Date: Mar 20, 2018
    Description: The Unsorted table map class from source code, edited to output correct information for project.
...
public class UnsortedTableMap<K,V> extends AbstractMap<K,V> {
  ...
  protected int getMaxClusterSize() { return maxClusterSize; } // some getters for implementation.
  ...
  public V put(K key, V value) {
    ...
    if (j == -1) {
      ...
      if(table.size() > 1){
        maxClusterSize = Math.max(maxClusterSize, table.size());
      }
      return null;
